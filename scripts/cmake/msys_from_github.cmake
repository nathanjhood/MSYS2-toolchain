...
function(msys_from_github)
    ...
    if(DEFINED arg_UNPARSED_ARGUMENTS)
        message(WARNING "msys_from_github was passed extra arguments: ${arg_UNPARSED_ARGUMENTS}")
    endif()
    ...
    if(MSYS_USE_HEAD_VERSION AND NOT DEFINED arg_HEAD_REF)
        message(STATUS "Package does not specify HEAD_REF. Falling back to non-HEAD version.")
        set(MSYS_USE_HEAD_VERSION OFF)
    elseif(NOT MSYS_USE_HEAD_VERSION AND NOT DEFINED arg_REF)
        message(FATAL_ERROR "Package does not specify REF. It must be built using --head.")
    endif()

    # exports MSYS_HEAD_VERSION to the caller. This will get picked up by ports.cmake after the build.
    if(MSYS_USE_HEAD_VERSION)
        string(REPLACE "/" "_-" sanitized_head_ref "${arg_HEAD_REF}")
        msys_download_distfile(archive_version
...
")
        ...
        set(MSYS_HEAD_VERSION "${head_version}" PARENT_SCOPE)
        set(ref_to_use "${head_version}")

        msys_list(SET redownload_param ALWAYS_REDOWNLOAD)
        msys_list(SET sha512_param SKIP_SHA512)
        msys_list(SET working_directory_param WORKING_DIRECTORY "${CURRENT_BUILDTREES_DIR}/src/head")
        msys_list(SET skip_patch_check_param SKIP_PATCH_CHECK)
    else()
        set(ref_to_use "${arg_REF}")

        msys_list(SET redownload_param)
        msys_list(SET working_directory_param)
        msys_list(SET skip_patch_check_param)
        msys_list(SET sha512_param SHA512 "${arg_SHA512}")
    endif()

    string(REPLACE "/" "_-" sanitized_ref "${ref_to_use}")
    if(DEFINED arg_FILE_DISAMBIGUATOR AND NOT MSYS_USE_HEAD_REF)
    ...
    msys_download_distfile(archive
    ...
    msys_extract_source_archive_ex(
