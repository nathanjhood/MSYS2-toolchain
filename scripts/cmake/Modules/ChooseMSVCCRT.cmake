...
Strips off the leading slash and any whitespace.
]===]
...
macro(choose_msvc_crt MSVC_CRT)
    ...
    if(MSYS_USE_CRT)
        ...
        message(FATAL_ERROR "MSYS_USE_CRT is deprecated. Use the CMAKE_BUILD_TYPE-specific variables (MSYS_USE_CRT_DEBUG, etc) instead.")
    ...
    foreach(build_type ${CMAKE_CONFIGURATION_TYPES} ${CMAKE_BUILD_TYPE})
        ...
        if (NOT MSYS_USE_CRT_${build})
            ...
            set(MSYS_USE_CRT_${build} "${MSYS_USE_CRT_${build}}" CACHE STRING "Specify VC++ CRT to use for ${build_type} configurations." FORCE)

            set_property(CACHE MSYS_USE_CRT_${build} PROPERTY STRINGS ;${${MSVC_CRT}})

        endif(NOT MSYS_USE_CRT_${build})
    ...
    foreach(build_type ${CMAKE_CONFIGURATION_TYPES} ${CMAKE_BUILD_TYPE})
        ...
        if ("${MSYS_USE_CRT_${build}}" STREQUAL "")

            set(flag_string " ")

        else()

            set(flag_string " /${MSYS_USE_CRT_${build}} ")

            if (NOT ${MSYS_USE_CRT_${build}} IN_LIST ${MSVC_CRT})

                message(FATAL_ERROR "Invalid value for MSYS_USE_CRT_${build}: ${MSYS_USE_CRT_${build}}. Valid options are one of: ${${MSVC_CRT}}")

            endif()

            message(STATUS "Using ${build_type} VC++ CRT: ${MSYS_USE_CRT_${build}}")
...
set(MSVC_CRT MD MDd MT MTd)
list(APPEND MSVC_CRT MD)
list(APPEND MSVC_CRT MDd)
list(APPEND MSVC_CRT MT)
list(APPEND MSVC_CRT MTd)
set(MSVC_CRT "${MSVC_CRT}" CACHE STRING "List of valid CRTs for MSVC." FORCE)

choose_msvc_crt(MSVC_CRT)

#[===[

The macro choose_msvc_crt() takes a list of possible C runtimes to choose from, in the
form of compiler flags, to present to the user. (MTd for /MTd, etc)

The macro should be invoked at the end of the file.
...
override that for each build type:
    ...
    1. Detect which CRT is already selected, and reflect this in LLVM_USE_CRT_* so the
    user can have a better idea of what changes they're making.

    2. Replace the flags in both variables with the new flag via a regex.

    3. set() the variables back into the cache so the changes are user-visible.
