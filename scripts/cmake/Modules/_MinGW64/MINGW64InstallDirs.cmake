...
macro(_MINGW64InstallDirs_cache_convert_to_path var description)
    get_property(_MINGW64InstallDirs_cache_type CACHE ${var} PROPERTY TYPE)
    if(_MINGW64InstallDirs_cache_type STREQUAL "UNINITIALIZED")
        file(TO_CMAKE_PATH "${${var}}" _MINGW64InstallDirs_cmakepath)
        set_property(CACHE ${var} PROPERTY TYPE PATH)
        set_property(CACHE ${var} PROPERTY VALUE "${_MINGW64InstallDirs_cmakepath}")
        set_property(CACHE ${var} PROPERTY HELPSTRING "${description}")
        unset(_MINGW64InstallDirs_cmakepath)
    endif()
    unset(_MINGW64InstallDirs_cache_type)
endmacro()

# Create a cache variable with default for a path.
macro(_MINGW64InstallDirs_cache_path var default description)
    if(NOT DEFINED ${var})
        set(${var} "${default}" CACHE PATH "${description}")
    endif()
    _MINGW64InstallDirs_cache_convert_to_path("${var}" "${description}")
endmacro()
...
# DATAROOTDIR.
macro(_MINGW64InstallDirs_cache_path_fallback var default description)
    if(NOT ${var})
...
_MINGW64InstallDirs_cache_path(MINGW64_INSTALL_BINDIR "bin"
  "User executables (bin)")
_MINGW64InstallDirs_cache_path(MINGW64_INSTALL_SBINDIR "sbin"
  "System admin executables (sbin)")
_MINGW64InstallDirs_cache_path(MINGW64_INSTALL_LIBEXECDIR "libexec"
  "Program executables (libexec)")
_MINGW64InstallDirs_cache_path(MINGW64_INSTALL_SYSCONFDIR "etc"
  "Read-only single-machine data (etc)")
_MINGW64InstallDirs_cache_path(MINGW64_INSTALL_SHAREDSTATEDIR "com"
  "Modifiable architecture-independent data (com)")
_MINGW64InstallDirs_cache_path(MINGW64_INSTALL_LOCALSTATEDIR "var"
  "Modifiable single-machine data (var)")
...
get_property(_libdir_set CACHE MINGW64_INSTALL_LIBDIR PROPERTY TYPE SET)
if(NOT DEFINED MINGW64_INSTALL_LIBDIR OR (_libdir_set
    AND DEFINED _MINGW64InstallDirs_LAST_MINGW64_INSTALL_PREFIX
    AND NOT "${_MINGW64InstallDirs_LAST_MINGW64_INSTALL_PREFIX}" STREQUAL "${MINGW64_INSTALL_PREFIX}"))
    ...
    if(DEFINED _MINGW64InstallDirs_LAST_MINGW64_INSTALL_PREFIX)
    ...
    if(CMAKE_SYSTEM_NAME MATCHES "^(Linux|kFreeBSD|GNU)$" AND NOT CMAKE_CROSSCOMPILING)
        ...
        if(DEFINED ENV{CONDA_BUILD} AND DEFINED ENV{PREFIX})
            ...
            if("${MINGW64_INSTALL_PREFIX}" STREQUAL conda_prefix)
                set(__system_type_for_install "conda")
            endif()
        elseif(DEFINED ENV{CONDA_PREFIX})
            set(conda_prefix "$ENV{CONDA_PREFIX}")
            cmake_path(ABSOLUTE_PATH conda_prefix NORMALIZE)
            if("${MINGW64_INSTALL_PREFIX}" STREQUAL conda_prefix AND
                NOT ("${MINGW64_INSTALL_PREFIX}" MATCHES "^/usr/?$" OR
                    "${MINGW64_INSTALL_PREFIX}" MATCHES "^/usr/local/?$"))
                set(__system_type_for_install "conda")
            endif()
        endif()
        ...
        if(__system_type_for_install STREQUAL "debian")
            if(MINGW64_LIBRARY_ARCHITECTURE)
                if("${MINGW64_INSTALL_PREFIX}" MATCHES "^/usr/?$")
                    set(_LIBDIR_DEFAULT "lib/${MINGW64_LIBRARY_ARCHITECTURE}")
                endif()
                if(DEFINED _MINGW64InstallDirs_LAST_MINGW64_INSTALL_PREFIX
                    AND "${_MINGW64InstallDirs_LAST_MINGW64_INSTALL_PREFIX}" MATCHES "^/usr/?$")
                    set(__LAST_LIBDIR_DEFAULT "lib/${MINGW64_LIBRARY_ARCHITECTURE}")
                endif()
            endif()
    ...
    if(NOT DEFINED MINGW64_INSTALL_LIBDIR)
        set(MINGW64_INSTALL_LIBDIR "${_LIBDIR_DEFAULT}" CACHE PATH "Object code libraries (${_LIBDIR_DEFAULT})")
    elseif(DEFINED __LAST_LIBDIR_DEFAULT AND "${__LAST_LIBDIR_DEFAULT}" STREQUAL "${MINGW64_INSTALL_LIBDIR}")
        set_property(CACHE MINGW64_INSTALL_LIBDIR PROPERTY VALUE "${_LIBDIR_DEFAULT}")
    endif()
endif()
_MINGW64InstallDirs_cache_convert_to_path(MINGW64_INSTALL_LIBDIR "Object code libraries (lib)")

# Save for next run
set(_MINGW64InstallDirs_LAST_MINGW64_INSTALL_PREFIX "${MINGW64_INSTALL_PREFIX}" CACHE INTERNAL "MINGW64_INSTALL_PREFIX during last run")
unset(_libdir_set)
unset(__LAST_LIBDIR_DEFAULT)

_MINGW64InstallDirs_cache_path(MINGW64_INSTALL_INCLUDEDIR "include"
  "C header files (include)")
_MINGW64InstallDirs_cache_path(MINGW64_INSTALL_OLDINCLUDEDIR "/usr/include"
  "C header files for non-gcc (/usr/include)")
_MINGW64InstallDirs_cache_path(MINGW64_INSTALL_DATAROOTDIR "share"
  "Read-only architecture-independent data root (share)")
...
_MINGW64InstallDirs_cache_path_fallback(MINGW64_INSTALL_DATADIR "${MINGW64_INSTALL_DATAROOTDIR}"
  "Read-only architecture-independent data (DATAROOTDIR)")

if(CMAKE_SYSTEM_NAME MATCHES "^(([^kF].*)?BSD|DragonFly)$")
    _MINGW64InstallDirs_cache_path_fallback(MINGW64_INSTALL_INFODIR "info"
    "Info documentation (info)")
else()
    _MINGW64InstallDirs_cache_path_fallback(MINGW64_INSTALL_INFODIR "${MINGW64_INSTALL_DATAROOTDIR}/info"
    "Info documentation (DATAROOTDIR/info)")
endif()

if(CMAKE_SYSTEM_NAME MATCHES "^(([^k].*)?BSD|DragonFly)$" AND NOT CMAKE_SYSTEM_NAME MATCHES "^(FreeBSD)$")
    _MINGW64InstallDirs_cache_path_fallback(MINGW64_INSTALL_MANDIR "man"
    "Man documentation (man)")
else()
    _MINGW64InstallDirs_cache_path_fallback(MINGW64_INSTALL_MANDIR "${MINGW64_INSTALL_DATAROOTDIR}/man"
    "Man documentation (DATAROOTDIR/man)")
endif()

_MINGW64InstallDirs_cache_path_fallback(MINGW64_INSTALL_LOCALEDIR "${MINGW64_INSTALL_DATAROOTDIR}/locale"
  "Locale-dependent data (DATAROOTDIR/locale)")
_MINGW64InstallDirs_cache_path_fallback(MINGW64_INSTALL_DOCDIR "${MINGW64_INSTALL_DATAROOTDIR}/doc/${PROJECT_NAME}"
  "Documentation root (DATAROOTDIR/doc/PROJECT_NAME)")

_MINGW64InstallDirs_cache_path_fallback(MINGW64_INSTALL_RUNSTATEDIR "${MINGW64_INSTALL_LOCALSTATEDIR}/run"
  "Run-time variable data (LOCALSTATEDIR/run)")

#-----------------------------------------------------------------------------

mark_as_advanced(
    MINGW64_INSTALL_BINDIR
    MINGW64_INSTALL_SBINDIR
    MINGW64_INSTALL_LIBEXECDIR
    MINGW64_INSTALL_SYSCONFDIR
    MINGW64_INSTALL_SHAREDSTATEDIR
    MINGW64_INSTALL_LOCALSTATEDIR
    MINGW64_INSTALL_RUNSTATEDIR
    MINGW64_INSTALL_LIBDIR
    MINGW64_INSTALL_INCLUDEDIR
    MINGW64_INSTALL_OLDINCLUDEDIR
    MINGW64_INSTALL_DATAROOTDIR
    MINGW64_INSTALL_DATADIR
    MINGW64_INSTALL_INFODIR
    MINGW64_INSTALL_LOCALEDIR
    MINGW64_INSTALL_MANDIR
    MINGW64_INSTALL_DOCDIR
)

macro(MINGW64InstallDirs_get_absolute_install_dir absvar var)
    ...
    if(NOT IS_ABSOLUTE "${${var}}")
        ...
        # - MINGW64_INSTALL_PREFIX == /
        # - MINGW64_INSTALL_PREFIX == /usr
        # - MINGW64_INSTALL_PREFIX == /opt/...
        if("${MINGW64_INSTALL_PREFIX}" STREQUAL "/")
        ...
        elseif("${MINGW64_INSTALL_PREFIX}" MATCHES "^/usr/?$")
            if("${GGAID_dir}" STREQUAL "SYSCONFDIR" OR "${GGAID_dir}" STREQUAL "LOCALSTATEDIR" OR "${GGAID_dir}" STREQUAL "RUNSTATEDIR")
                set(${absvar} "/${${var}}")
            else()
                set(${absvar} "${MINGW64_INSTALL_PREFIX}/${${var}}")
            endif()
        elseif("${MINGW64_INSTALL_PREFIX}" MATCHES "^/opt/" AND NOT "${MINGW64_INSTALL_PREFIX}" MATCHES "^/opt/homebrew/")
            if("${GGAID_dir}" STREQUAL "SYSCONFDIR" OR "${GGAID_dir}" STREQUAL "LOCALSTATEDIR" OR "${GGAID_dir}" STREQUAL "RUNSTATEDIR")
                set(${absvar} "/${${var}}${MINGW64_INSTALL_PREFIX}")
            else()
                set(${absvar} "${MINGW64_INSTALL_PREFIX}/${${var}}")
            endif()
        else()
...
foreach(dir
    ...
    MINGW64InstallDirs_get_absolute_install_dir(MINGW64_INSTALL_FULL_${dir} MINGW64_INSTALL_${dir} ${dir})
endforeach()
