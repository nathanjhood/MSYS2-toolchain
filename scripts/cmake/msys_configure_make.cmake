...
macro(z_msys_determine_autotools_host_cpu out_var)
    ...
    elseif(DEFINED ENV{PROCESSOR_ARCHITECTURE})
        ...
        set(host_arch "${MSYS_DETECTED_CMAKE_HOST_SYSTEM_PROCESSOR}")
    endif()
    ...
    elseif(host_arch MATCHES "^(ARM|arm)$")
        ...
        message(FATAL_ERROR "Unsupported host architecture ${host_arch} in z_msys_determine_autotools_host_cpu()!" )
    endif()
    unset(host_arch)
endmacro()

macro(z_msys_determine_autotools_target_cpu out_var)
    message("Calling ${CMAKE_CURRENT_FUNCTION}")

    if(MSYS_TARGET_ARCHITECTURE MATCHES "(x|X)64")
        set(${out_var} x86_64)
    elseif(MSYS_TARGET_ARCHITECTURE MATCHES "(x|X)86")
        set(${out_var} i686)
    elseif(MSYS_TARGET_ARCHITECTURE MATCHES "^(ARM|arm)64$")
        set(${out_var} aarch64)
    elseif(MSYS_TARGET_ARCHITECTURE MATCHES "^(ARM|arm)$")
        set(${out_var} arm)
    else()
        message(FATAL_ERROR "Unsupported MSYS_TARGET_ARCHITECTURE architecture ${MSYS_TARGET_ARCHITECTURE} in z_msys_determine_autotools_target_cpu()!" )
    endif()
endmacro()

macro(z_msys_set_arch_mac out_var value)
    message("Calling ${CMAKE_CURRENT_FUNCTION}")
...
macro(z_msys_determine_autotools_host_arch_mac out_var)
    message("Calling ${CMAKE_CURRENT_FUNCTION}")

    z_msys_set_arch_mac(${out_var} "${MSYS_DETECTED_CMAKE_HOST_SYSTEM_PROCESSOR}")
endmacro()

macro(z_msys_determine_autotools_target_arch_mac out_var)
    message("Calling ${CMAKE_CURRENT_FUNCTION}")

    list(LENGTH MSYS_OSX_ARCHITECTURES osx_archs_num)
    if(osx_archs_num EQUAL 0)
        z_msys_set_arch_mac(${out_var} "${MSYS_DETECTED_CMAKE_HOST_SYSTEM_PROCESSOR}")
    elseif(osx_archs_num GREATER_EQUAL 2)
        set(${out_var} "universal")
    else()
        z_msys_set_arch_mac(${out_var} "${MSYS_OSX_ARCHITECTURES}")
    endif()
    unset(osx_archs_num)
endmacro()

macro(z_msys_extract_cpp_flags_and_set_cflags_and_cxxflags flag_suffix)
    message("Calling ${CMAKE_CURRENT_FUNCTION}")

    string(REGEX MATCHALL "( |^)(-D|-isysroot|--sysroot=|-isystem|-m?[Aa][Rr][Cc][Hh]|--target=|-target) ?[^ ]+" CPPFLAGS_${flag_suffix} "${MSYS_DETECTED_CMAKE_C_FLAGS_${flag_suffix}}")
    string(REGEX MATCHALL "( |^)(-D|-isysroot|--sysroot=|-isystem|-m?[Aa][Rr][Cc][Hh]|--target=|-target) ?[^ ]+" CXXPPFLAGS_${flag_suffix} "${MSYS_DETECTED_CMAKE_CXX_FLAGS_${flag_suffix}}")
    ...
    if(CPPREGEX)
        string(REGEX REPLACE "(${CPPREGEX})" "" CFLAGS_${flag_suffix} "${MSYS_DETECTED_CMAKE_C_FLAGS_${flag_suffix}}")
        string(REGEX REPLACE "(${CPPREGEX})" "" CXXFLAGS_${flag_suffix} "${MSYS_DETECTED_CMAKE_CXX_FLAGS_${flag_suffix}}")
    else()
        set(CFLAGS_${flag_suffix} "${MSYS_DETECTED_CMAKE_C_FLAGS_${flag_suffix}}")
        set(CXXFLAGS_${flag_suffix} "${MSYS_DETECTED_CMAKE_CXX_FLAGS_${flag_suffix}}")
    endif()
    ...
    # just ignore them.
    if(MSYS_DETECTED_CMAKE_C_COMPILER_FRONTEND_VARIANT STREQUAL "MSVC" OR MSYS_DETECTED_CMAKE_C_COMPILER_ID STREQUAL "MSVC")
    ...
    if(MSYS_DETECTED_CMAKE_CXX_COMPILER_FRONTEND_VARIANT STREQUAL "MSVC" OR MSYS_DETECTED_CMAKE_CXX_COMPILER_ID STREQUAL "MSVC")
...
macro(z_msys_append_to_configure_environment inoutstring var defaultval)
    message("Calling ${CMAKE_CURRENT_FUNCTION}")
...
function(msys_configure_make)
    ...
    z_msys_get_cmake_vars(cmake_vars_file)
    ...
    foreach(var IN ITEMS MSYS_DETECTED_CMAKE_C_FLAGS_DEBUG
                         MSYS_DETECTED_CMAKE_C_FLAGS_RELEASE
                         MSYS_DETECTED_CMAKE_CXX_FLAGS_DEBUG
                         MSYS_DETECTED_CMAKE_CXX_FLAGS_RELEASE
                         MSYS_DETECTED_CMAKE_SHARED_LINKER_FLAGS_DEBUG
                         MSYS_DETECTED_CMAKE_SHARED_LINKER_FLAGS_RELEASE
                         MSYS_DETECTED_CMAKE_STATIC_LINKER_FLAGS_DEBUG
                         MSYS_DETECTED_CMAKE_STATIC_LINKER_FLAGS_RELEASE
                         MSYS_DETECTED_CMAKE_C_STANDARD_LIBRARIES
                         MSYS_DETECTED_CMAKE_CXX_STANDARD_LIBRARIES
    )
        separate_arguments(cmake_list NATIVE_COMMAND "${${var}}")
        list(JOIN cmake_list " " "${var}")
    endforeach()

    if(DEFINED MSYS_MAKE_BUILD_TRIPLET)
        set(arg_BUILD_TRIPLET ${MSYS_MAKE_BUILD_TRIPLET}) # Triplet overwrite for crosscompiling
    endif()
    ...
    if(CMAKE_HOST_WIN32 AND MSYS_DETECTED_CMAKE_C_COMPILER MATCHES "cl.exe") #only applies to windows (clang-)cl and lib
        if(arg_AUTOCONFIG)
    ...
    msys_backup_env_variables(VARS ${cm_FLAGS})
    ...
    #Used by gcc/linux
    msys_backup_env_variables(VARS C_INCLUDE_PATH CPLUS_INCLUDE_PATH LIBRARY_PATH LD_LIBRARY_PATH)

    #Used by cl
    msys_backup_env_variables(VARS INCLUDE LIB LIBPATH)
    ...
    if(CMAKE_HOST_WIN32)
        ...
        msys_acquire_msys(MSYS_ROOT PACKAGES ${msys_require_packages} ${arg_ADDITIONAL_MSYS_PACKAGES})
        set(base_cmd "${MSYS_ROOT}/usr/bin/bash.exe" --noprofile --norc --debug)
        msys_list(SET add_to_env)
        if(arg_USE_WRAPPERS AND MSYS_TARGET_IS_WINDOWS)
            msys_list(APPEND add_to_env "${SCRIPTS}/buildsystems/make_wrapper") # Other required wrappers are also located there
            msys_list(APPEND add_to_env "${MSYS_ROOT}/usr/share/automake-1.16")
        endif()
        ...
        msys_list(SET find_system_dirs
        ...
        msys_list(INSERT path_list "${index}" ${add_to_env} "${MSYS_ROOT}/usr/bin")
   ...
   # macOS - cross-compiling support
    if(MSYS_TARGET_IS_OSX OR MSYS_TARGET_IS_IOS)
        if (requires_autoconfig AND NOT arg_BUILD_TRIPLET OR arg_DETERMINE_BUILD_TRIPLET)
            z_msys_determine_autotools_host_arch_mac(BUILD_ARCH) # machine you are building on => --build=
            z_msys_determine_autotools_target_arch_mac(TARGET_ARCH)
            # --build: the machine you are building on
            # --host: the machine you are building for
            # --target: the machine that CC will produce binaries for
            # https://stackoverflow.com/questions/21990021/how-to-determine-host-value-for-configure-when-using-cross-compiler
            # Only for ports using autotools so we can assume that they follow the common conventions for build/target/host
            if(NOT "${TARGET_ARCH}" STREQUAL "${BUILD_ARCH}" OR NOT MSYS_TARGET_IS_OSX) # we don't need to specify the additional flags if we build natively.
    ...
    if(MSYS_TARGET_IS_LINUX)
        if (requires_autoconfig AND NOT arg_BUILD_TRIPLET OR arg_DETERMINE_BUILD_TRIPLET)
            ...
            if(MSYS_DETECTED_CMAKE_C_COMPILER MATCHES "([^\/]*)-gcc$" AND CMAKE_MATCH_1)
    ...
    if (MSYS_TARGET_IS_WINDOWS)
        if (arg_DETERMINE_BUILD_TRIPLET OR NOT arg_BUILD_TRIPLET)
            z_msys_determine_autotools_host_cpu(BUILD_ARCH) # MSYS_HOST => machine you are building on => --build=
            z_msys_determine_autotools_target_cpu(TARGET_ARCH)
            # --build: the machine you are building on
            # --host: the machine you are building for
            # --target: the machine that CC will produce binaries for
            # https://stackoverflow.com/questions/21990021/how-to-determine-host-value-for-configure-when-using-cross-compiler
            ...
            if(MSYS_TARGET_IS_UWP AND NOT arg_BUILD_TRIPLET MATCHES "--host")
        ...
        set(progs MSYS_DETECTED_CMAKE_C_COMPILER MSYS_DETECTED_CMAKE_CXX_COMPILER MSYS_DETECTED_CMAKE_AR
                  MSYS_DETECTED_CMAKE_LINKER MSYS_DETECTED_CMAKE_RANLIB MSYS_DETECTED_CMAKE_OBJDUMP
                  MSYS_DETECTED_CMAKE_STRIP MSYS_DETECTED_CMAKE_NM MSYS_DETECTED_CMAKE_DLLTOOL MSYS_DETECTED_CMAKE_RC_COMPILER)
        foreach(prog IN LISTS progs)
            if(${prog})
                ...
                if(NOT path STREQUAL prog_found)
                    ...
                    msys_add_to_path(PREPEND ${path})
                endif()
            endif()
        endforeach()
        if (arg_USE_WRAPPERS)
            z_msys_append_to_configure_environment(configure_env CPP "compile ${MSYS_DETECTED_CMAKE_C_COMPILER} -E")

            z_msys_append_to_configure_environment(configure_env CC "compile ${MSYS_DETECTED_CMAKE_C_COMPILER}")
            if(NOT arg_BUILD_TRIPLET MATCHES "--host")
                z_msys_append_to_configure_environment(configure_env CC_FOR_BUILD "compile ${MSYS_DETECTED_CMAKE_C_COMPILER}")
                z_msys_append_to_configure_environment(configure_env CPP_FOR_BUILD "compile ${MSYS_DETECTED_CMAKE_C_COMPILER} -E")
                z_msys_append_to_configure_environment(configure_env CXX_FOR_BUILD "compile ${MSYS_DETECTED_CMAKE_CXX_COMPILER}")
            else()
                # Silly trick to make configure accept CC_FOR_BUILD but in reallity CC_FOR_BUILD is deactivated.
                z_msys_append_to_configure_environment(configure_env CC_FOR_BUILD "touch a.out | touch conftest${MSYS_HOST_EXECUTABLE_SUFFIX} | true")
                z_msys_append_to_configure_environment(configure_env CPP_FOR_BUILD "touch a.out | touch conftest${MSYS_HOST_EXECUTABLE_SUFFIX} | true")
                z_msys_append_to_configure_environment(configure_env CXX_FOR_BUILD "touch a.out | touch conftest${MSYS_HOST_EXECUTABLE_SUFFIX} | true")
            endif()
            z_msys_append_to_configure_environment(configure_env CXX "compile ${MSYS_DETECTED_CMAKE_CXX_COMPILER}")
            z_msys_append_to_configure_environment(configure_env RC "windres-rc ${MSYS_DETECTED_CMAKE_RC_COMPILER}")
            z_msys_append_to_configure_environment(configure_env WINDRES "windres-rc ${MSYS_DETECTED_CMAKE_RC_COMPILER}")
            if(MSYS_DETECTED_CMAKE_AR)
                z_msys_append_to_configure_environment(configure_env AR "ar-lib ${MSYS_DETECTED_CMAKE_AR}")
            else()
                z_msys_append_to_configure_environment(configure_env AR "ar-lib lib.exe -verbose")
            endif()
        else()
            z_msys_append_to_configure_environment(configure_env CPP "${MSYS_DETECTED_CMAKE_C_COMPILER} -E")
            z_msys_append_to_configure_environment(configure_env CC "${MSYS_DETECTED_CMAKE_C_COMPILER}")
            if(NOT arg_BUILD_TRIPLET MATCHES "--host")
                z_msys_append_to_configure_environment(configure_env CC_FOR_BUILD "${MSYS_DETECTED_CMAKE_C_COMPILER}")
                z_msys_append_to_configure_environment(configure_env CPP_FOR_BUILD "${MSYS_DETECTED_CMAKE_C_COMPILER} -E")
                z_msys_append_to_configure_environment(configure_env CXX_FOR_BUILD "${MSYS_DETECTED_CMAKE_CXX_COMPILER}")
            else()
                z_msys_append_to_configure_environment(configure_env CC_FOR_BUILD "touch a.out | touch conftest${MSYS_HOST_EXECUTABLE_SUFFIX} | true")
                z_msys_append_to_configure_environment(configure_env CPP_FOR_BUILD "touch a.out | touch conftest${MSYS_HOST_EXECUTABLE_SUFFIX} | true")
                z_msys_append_to_configure_environment(configure_env CXX_FOR_BUILD "touch a.out | touch conftest${MSYS_HOST_EXECUTABLE_SUFFIX} | true")
            endif()
            z_msys_append_to_configure_environment(configure_env CXX "${MSYS_DETECTED_CMAKE_CXX_COMPILER}")
            z_msys_append_to_configure_environment(configure_env RC "${MSYS_DETECTED_CMAKE_RC_COMPILER}")
            z_msys_append_to_configure_environment(configure_env WINDRES "${MSYS_DETECTED_CMAKE_RC_COMPILER}")
            if(MSYS_DETECTED_CMAKE_AR)
                z_msys_append_to_configure_environment(configure_env AR "${MSYS_DETECTED_CMAKE_AR}")
            else()
                z_msys_append_to_configure_environment(configure_env AR "lib.exe -verbose")
            endif()
        endif()
        z_msys_append_to_configure_environment(configure_env LD "${MSYS_DETECTED_CMAKE_LINKER} -verbose")
        if(MSYS_DETECTED_CMAKE_RANLIB)
            z_msys_append_to_configure_environment(configure_env RANLIB "${MSYS_DETECTED_CMAKE_RANLIB}") # Trick to ignore the RANLIB call
        else()
            z_msys_append_to_configure_environment(configure_env RANLIB ":")
        endif()
        if(MSYS_DETECTED_CMAKE_OBJDUMP) #Objdump is required to make shared libraries. Otherwise define lt_cv_deplibs_check_method=pass_all
            z_msys_append_to_configure_environment(configure_env OBJDUMP "${MSYS_DETECTED_CMAKE_OBJDUMP}") # Trick to ignore the RANLIB call
        endif()
        if(MSYS_DETECTED_CMAKE_STRIP) # If required set the ENV variable STRIP in the portfile correctly
            z_msys_append_to_configure_environment(configure_env STRIP "${MSYS_DETECTED_CMAKE_STRIP}")
        else()
            z_msys_append_to_configure_environment(configure_env STRIP ":")
            list(APPEND arg_OPTIONS ac_cv_prog_ac_ct_STRIP=:)
        endif()
        if(MSYS_DETECTED_CMAKE_NM) # If required set the ENV variable NM in the portfile correctly
            z_msys_append_to_configure_environment(configure_env NM "${MSYS_DETECTED_CMAKE_NM}")
        else()
            ...
            z_msys_append_to_configure_environment(configure_env NM "dumpbin.exe -symbols -headers")
        endif()
        if(MSYS_DETECTED_CMAKE_DLLTOOL) # If required set the ENV variable DLLTOOL in the portfile correctly
            z_msys_append_to_configure_environment(configure_env DLLTOOL "${MSYS_DETECTED_CMAKE_DLLTOOL}")
        else()
            z_msys_append_to_configure_environment(configure_env DLLTOOL "link.exe -verbose -dll")
        endif()
        z_msys_append_to_configure_environment(configure_env CCAS ":")   # If required set the ENV variable CCAS in the portfile correctly
        z_msys_append_to_configure_environment(configure_env AS ":")   # If required set the ENV variable AS in the portfile correctly

        foreach(_env IN LISTS arg_CONFIGURE_ENVIRONMENT_VARIABLES)
            z_msys_append_to_configure_environment(configure_env ${_env} "${${_env}}")
        endforeach()
        ...
        if(MSYS_TARGET_ARCHITECTURE MATCHES "^[Aa][Rr][Mm]64$")
        ...
        elseif(MSYS_TARGET_ARCHITECTURE MATCHES "^[Aa][Rr][Mm]$")
        ...
        function(z_msys_make_set_env envvar cmakevar)
            set(prog "${MSYS_DETECTED_CMAKE_${cmakevar}} ${ARGN}")
            string(STRIP "${prog}" prog)
            if(DEFINED ENV{${envvar}})
                return()
            endif()
            if(MSYS_DETECTED_CMAKE_${cmakevar})
                set(ENV{${envvar}} "${prog}")
            endif()
        endfunction()
        z_msys_make_set_env(CC C_COMPILER)
        if(NOT arg_BUILD_TRIPLET MATCHES "--host")
            z_msys_make_set_env(CC_FOR_BUILD C_COMPILER)
            z_msys_make_set_env(CPP_FOR_BUILD C_COMPILER "-E")
            z_msys_make_set_env(CXX_FOR_BUILD C_COMPILER)
        else()
            set(ENV{CC_FOR_BUILD} "touch a.out | touch conftest${MSYS_HOST_EXECUTABLE_SUFFIX} | true")
            set(ENV{CPP_FOR_BUILD} "touch a.out | touch conftest${MSYS_HOST_EXECUTABLE_SUFFIX} | true")
            set(ENV{CXX_FOR_BUILD} "touch a.out | touch conftest${MSYS_HOST_EXECUTABLE_SUFFIX} | true")
        endif()
        z_msys_make_set_env(CXX CXX_COMPILER)
        z_msys_make_set_env(NM NM)
        z_msys_make_set_env(RC RC)
        z_msys_make_set_env(WINDRES RC)
        z_msys_make_set_env(DLLTOOL DLLTOOL)
        z_msys_make_set_env(STRIP STRIP)
        z_msys_make_set_env(OBJDUMP OBJDUMP)
        z_msys_make_set_env(RANLIB RANLIB)
        z_msys_make_set_env(AR AR)
        z_msys_make_set_env(LD LINKER)
        unset(z_msys_make_set_env)
    endif()
    ...
    # z_msys_installed_path:      Native path with escaped space characters
    # z_msys_prefix_path:         Path with unprotected spaces, but drive letters transformed for mingw/msys
    string(REPLACE " " "\\ " z_msys_installed_path "${CURRENT_INSTALLED_DIR}")
    if(CMAKE_HOST_WIN32)
        string(REGEX REPLACE "([a-zA-Z]):/" "/\\1/" z_msys_prefix_path "${CURRENT_INSTALLED_DIR}")
    else()
        set(z_msys_prefix_path "${CURRENT_INSTALLED_DIR}")
    endif()
    ...
    # Set configure paths
    set(arg_OPTIONS_RELEASE ${arg_OPTIONS_RELEASE} "--prefix=${z_msys_prefix_path}")
    set(arg_OPTIONS_DEBUG ${arg_OPTIONS_DEBUG} "--prefix=${z_msys_prefix_path}/debug")
    ...
    if(DEFINED MSYS_CONFIGURE_MAKE_OPTIONS)
        list(APPEND arg_OPTIONS ${MSYS_CONFIGURE_MAKE_OPTIONS})
    endif()
    if(DEFINED MSYS_CONFIGURE_MAKE_OPTIONS_RELEASE)
        list(APPEND arg_OPTIONS_RELEASE ${MSYS_CONFIGURE_MAKE_OPTIONS_RELEASE})
    endif()
    if(DEFINED MSYS_CONFIGURE_MAKE_OPTIONS_DEBUG)
        list(APPEND arg_OPTIONS_DEBUG ${MSYS_CONFIGURE_MAKE_OPTIONS_DEBUG})
    endif()

    file(RELATIVE_PATH relative_build_path "${CURRENT_BUILDTREES_DIR}" "${arg_SOURCE_PATH}/${arg_PROJECT_SUBPATH}")

    # Used by CL
    msys_host_path_list(PREPEND ENV{INCLUDE} "${CURRENT_INSTALLED_DIR}/include")
    # Used by GCC
    msys_host_path_list(PREPEND ENV{C_INCLUDE_PATH} "${CURRENT_INSTALLED_DIR}/include")
    msys_host_path_list(PREPEND ENV{CPLUS_INCLUDE_PATH} "${CURRENT_INSTALLED_DIR}/include")

    # Flags should be set in the toolchain instead (Setting this up correctly requires a function named msys_determined_cmake_compiler_flags which can also be used to setup CC and CXX etc.)
    if(MSYS_TARGET_IS_WINDOWS)
        msys_backup_env_variables(VARS _CL_ _LINK_)
        # TODO: Should be CPP flags instead -> rewrite when msys_determined_cmake_compiler_flags defined
        if(MSYS_TARGET_IS_UWP)
            ...
            string(REPLACE "${_replacement}" "" MSYS_DETECTED_CMAKE_CXX_FLAGS_DEBUG "${MSYS_DETECTED_CMAKE_CXX_FLAGS_DEBUG}")
            string(REPLACE "${_replacement}" "" MSYS_DETECTED_CMAKE_C_FLAGS_DEBUG "${MSYS_DETECTED_CMAKE_C_FLAGS_DEBUG}")
            string(REPLACE "${_replacement}" "" MSYS_DETECTED_CMAKE_CXX_FLAGS_RELEASE "${MSYS_DETECTED_CMAKE_CXX_FLAGS_RELEASE}")
            string(REPLACE "${_replacement}" "" MSYS_DETECTED_CMAKE_C_FLAGS_RELEASE "${MSYS_DETECTED_CMAKE_C_FLAGS_RELEASE}")
            ...
            set(ENV{_LINK_} "$ENV{_LINK_} ${MSYS_DETECTED_CMAKE_C_STANDARD_LIBRARIES} ${MSYS_DETECTED_CMAKE_CXX_STANDARD_LIBRARIES}")
        endif()
    endif()

    z_convert_to_list(MSYS_DETECTED_CMAKE_C_STANDARD_LIBRARIES c_libs_list)
    z_convert_to_list(MSYS_DETECTED_CMAKE_CXX_STANDARD_LIBRARIES cxx_libs_list)
    ...
    set(x_msys_transform_libs ON)
    if(MSYS_TARGET_IS_UWP)
        set(x_msys_transform_libs OFF)
    ...
    if(MSYS_TARGET_IS_MINGW AND CRT_LINKAGE STREQUAL "dynamic")
    ...
    macro(z_msys_setup_make_linker_flags_vars var_suffix)
        if(CRT_LINKAGE STREQUAL "static")
            set(LINKER_FLAGS_${var_suffix} "${MSYS_DETECTED_CMAKE_STATIC_LINKER_FLAGS_${var_suffix}}")
        else() # dynamic
            set(LINKER_FLAGS_${var_suffix} "${MSYS_DETECTED_CMAKE_SHARED_LINKER_FLAGS_${var_suffix}}")
        endif()
        set(ARFLAGS_${var_suffix} "${MSYS_DETECTED_CMAKE_STATIC_LINKER_FLAGS_${var_suffix}}")
        set(LDFLAGS_${var_suffix} "${MSYS_DETECTED_CMAKE_SHARED_LINKER_FLAGS_${var_suffix}}")
        if (CMAKE_HOST_WIN32 AND MSYS_DETECTED_CMAKE_C_COMPILER_ID MATCHES [[cl\.exe$]])
            if(NOT vcm_paths_with_spaces)
                string(APPEND LDFLAGS_${var_suffix} " -L${z_msys_installed_path}${path_suffix_${var_suffix}}/lib -L${z_msys_installed_path}${path_suffix_${var_suffix}}/lib/manual-link")
            endif()
            ...
            if(EXISTS "${CURRENT_INSTALLED_DIR}${path_suffix_${var_suffix}}/lib")
                set(link_required_dirs "-L${z_msys_installed_path}${path_suffix_${var_suffix}}/lib")
            endif()
            if(EXISTS "{CURRENT_INSTALLED_DIR}${path_suffix_${var_suffix}}/lib/manual-link")
                set(link_required_dirs "${link_required_dirs} -L${z_msys_installed_path}${path_suffix_${var_suffix}}/lib/manual-link")
            endif()
            ...
            if(MSYS_TARGET_IS_WINDOWS AND MSYS_DETECTED_CMAKE_LINKER MATCHES [[link\.exe$]])
    ...
    if(NOT DEFINED MSYS_BUILD_TYPE OR MSYS_BUILD_TYPE STREQUAL "debug" AND NOT arg_NO_DEBUG)
        set(var_suffix DEBUG)
        set(path_suffix_${var_suffix} "/debug")
        set(short_name_${var_suffix} "dbg")
        list(APPEND all_buildtypes ${var_suffix})
        z_msys_extract_cpp_flags_and_set_cflags_and_cxxflags(${var_suffix})
        z_msys_setup_make_linker_flags_vars(${var_suffix})
        unset(var_suffix)
    endif()
    ...
    foreach(var IN ITEMS arg_OPTIONS arg_OPTIONS_RELEASE arg_OPTIONS_DEBUG)
        ...
        foreach(element IN LISTS "${var}")
            ...
            msys_list(APPEND tmp "\"${element}\"")
        endforeach()
        msys_list(JOIN tmp " " "${var}")
    endforeach()
    ...
    foreach(current_buildtype IN LISTS all_buildtypes)
        ...
        if ("${current_buildtype}" STREQUAL "DEBUG")
            z_msys_setup_pkgconfig_path(BASE_DIRS "${CURRENT_INSTALLED_DIR}/debug")
        else()
            z_msys_setup_pkgconfig_path(BASE_DIRS "${CURRENT_INSTALLED_DIR}")
        endif()
        ...
        set(ENV{RCFLAGS} "${MSYS_DETECTED_CMAKE_RC_FLAGS_${current_buildtype}}")
        set(ENV{LDFLAGS} "${LDFLAGS_${current_buildtype}}")
        set(ENV{LDFLAGS_FOR_BUILD} "${LDFLAGS_${current_buildtype}}")
        if(ARFLAGS_${current_buildtype} AND NOT (arg_USE_WRAPPERS AND MSYS_TARGET_IS_WINDOWS))
        ...
        msys_list(APPEND lib_env_vars LIB LIBPATH LIBRARY_PATH) # LD_LIBRARY_PATH)
        foreach(lib_env_var IN LISTS lib_env_vars)
            if(EXISTS "${CURRENT_INSTALLED_DIR}${path_suffix_${current_buildtype}}/lib")
                msys_host_path_list(PREPEND ENV{${lib_env_var}} "${CURRENT_INSTALLED_DIR}${path_suffix_${current_buildtype}}/lib")
            endif()
            if(EXISTS "${CURRENT_INSTALLED_DIR}${path_suffix_${current_buildtype}}/lib/manual-link")
                msys_host_path_list(PREPEND ENV{${lib_env_var}} "${CURRENT_INSTALLED_DIR}${path_suffix_${current_buildtype}}/lib/manual-link")
            endif()
        endforeach()
        unset(lib_env_vars)
        ...
        if(arg_ADD_BIN_TO_PATH)
            ...
            msys_add_to_path("${CURRENT_INSTALLED_DIR}${path_suffix_${current_buildtype}}/bin")
        endif()
        ...
        if (NOT arg_SKIP_CONFIGURE)
            ...
            if(MSYS_TARGET_IS_WINDOWS AND NOT MSYS_TARGET_IS_MINGW AND CRT_LINKAGE STREQUAL dynamic)
    ...
    # Export matching make program for msys_build_make (cache variable)
    if(CMAKE_HOST_WIN32 AND MSYS_ROOT)
        find_program(Z_MSYS_MAKE make PATHS "${MSYS_ROOT}/usr/bin" NO_DEFAULT_PATH REQUIRED)
    elseif(MSYS_HOST_IS_OPENBSD)
        find_program(Z_MSYS_MAKE gmake REQUIRED)
    else()
        find_program(Z_MSYS_MAKE make REQUIRED)
    endif()

    # Restore environment
    msys_restore_env_variables(VARS ${cm_FLAGS} LIB LIBPATH LIBRARY_PATH LD_LIBRARY_PATH)

    set(_MSYS_PROJECT_SOURCE_PATH ${arg_SOURCE_PATH} PARENT_SCOPE)
    set(_MSYS_PROJECT_SUBPATH ${arg_PROJECT_SUBPATH} PARENT_SCOPE)
    set(_MSYS_MAKE_NO_DEBUG ${arg_NO_DEBUG} PARENT_SCOPE)
endfunction()
