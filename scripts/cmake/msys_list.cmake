...
macro(z_msys_list_escape_once_more lst)
...
function(msys_list)
    ...
    # msys_list(<operation> <list_var> ...)
    #            A0          A1

    if(ARGC LESS "2")
        message(FATAL_ERROR "msys_list requires at least two arguments.")
    endif()

    if(ARGV1 MATCHES "^ARGV([0-9]*)$|^ARG[CN]$|^CMAKE_CURRENT_FUNCTION")
        message(FATAL_ERROR "msys_list does not support the list_var being ${ARGV1}.
    ...
    if(operation STREQUAL "SET")
        z_msys_function_arguments(args 2)
    ...
    if(operation STREQUAL "LENGTH")
        # msys_list(LENGTH <list-var> <out-var>)
        #            A0     A1         A2
        if(NOT ARGC EQUAL "3")
            message(FATAL_ERROR "msys_list sub-command ${operation} requires two arguments.")
        endif()
    ...
    if(operation MATCHES "^(GET|JOIN|FIND)$")
        # msys_list(<operation> <list-var> <arg> <out-var>)
        #            A0          A1         A2    A3
        if(NOT ARGC EQUAL "4")
            message(FATAL_ERROR "msys_list sub-command ${operation} requires three arguments.")
        endif()
        if(operation STREQUAL "GET")
            list(LENGTH list length)
            if(length EQUAL "0")
                message(FATAL_ERROR "msys_list GET given empty list")
            elseif(ARGV2 GREATER_EQUAL length OR ARGV2 LESS "-${length}")
                message(FATAL_ERROR "msys_list index: ${ARGV2} is not in range")
            endif()
        endif()
    ...
    if(operation STREQUAL "SUBLIST")
        # msys_list(SUBLIST <list-var> <begin> <length> <out-var>)
        #            A0      A1         A2      A3       A4
        if(NOT ARGC EQUAL "5")
            message(FATAL_ERROR "msys_list sub-command SUBLIST requires four arguments.")
        endif()
        list(LENGTH list length)
        if(ARGV2 LESS "0" OR (ARGV2 GREATER_EQUAL length AND NOT ARGV2 EQUAL "0"))
            message(FATAL_ERROR "msys_list begin index: ${ARGV2} is out of range")
        endif()
        z_msys_list_escape_once_more(list)
    ...
    if(operation MATCHES "^(APPEND|PREPEND)$")
        # msys_list(<operation> <list> [<element>...])
        #            A0          A1      A2...
    ...
    if(operation STREQUAL "INSERT")
        # msys_list(INSERT <list> <index> [<element>...])
        #            A0     A1     A2       A3...
        ...
        if(ARGC GREATER 3)
            ...
            z_msys_function_arguments(args 3)
            if(list STREQUAL "")
                set("${list_var}" "${args}" PARENT_SCOPE)
            ...
            elseif(ARGV2 EQUAL length)
                ...
                msys_list(SUBLIST list 0 "${ARGV2}" list_start)
                msys_list(SUBLIST list "${ARGV2}" -1 list_end)
                set("${list_var}" "${list_start};${args};${list_end}" PARENT_SCOPE)
            endif()
        elseif(ARGC LESS 3)
            message(FATAL_ERROR "msys_list sub-command INSERT requires at least two arguments.")
        endif()
        return()
    endif()

    if(operation MATCHES "^(POP_BACK|POP_FRONT|REVERSE|REMOVE_DUPLICATES)$")
        # msys_list(<operation> <list>)
        #            A0          A1
        if(NOT ARGC EQUAL 2)
            message(FATAL_ERROR "msys_list sub-command ${operation} requires one argument.")
        endif()
        z_msys_list_escape_once_more(list)
        list("${operation}" list)
    ...
    if(operation MATCHES "^(REMOVE_AT|REMOVE_ITEM)$")
        # msys_list(<operation> <list> <index-or-item>)
        #            A0          A1     A2
        if(NOT ARGC EQUAL 3)
            message(FATAL_ERROR "msys_list sub-command ${operation} requires two arguments.")
        endif()
    ...
    message(FATAL_ERROR "msys_list sub-command ${operation} is not yet implemented.")
endfunction()
