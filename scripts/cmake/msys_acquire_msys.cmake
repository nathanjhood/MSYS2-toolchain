...
set(Z_MSYS_ACQUIRE_MSYS_MIRRORS
    # Alternative primary
...
#   - Z_msys_MSYS_PACKAGES_AVAILABLE
#   - Z_msys_MSYS_${arg_NAME}_URL
#   - Z_msys_MSYS_${arg_NAME}_SHA512
#   - Z_msys_MSYS_${arg_NAME}_FILENAME
#   - Z_msys_MSYS_${arg_NAME}_DEPS
#   - Z_msys_MSYS_${arg_NAME}_PATCHES
#   - Z_msys_MSYS_${arg_NAME}_DIRECT
...
function(z_msys_acquire_msys_declare_package)
    ...
    if(DEFINED arg_UNPARSED_ARGUMENTS)
        message(FATAL_ERROR "internal error: z_msys_acquire_msys_declare_package passed extra args: ${arg_UNPARSED_ARGUMENTS}")
    endif()
    foreach(required_arg IN ITEMS URL SHA512)
        if(NOT DEFINED arg_${required_arg})
            message(FATAL_ERROR "internal error: z_msys_acquire_msys_declare_package requires argument: ${required_arg}")
        endif()
    endforeach()

    if(arg_DIRECT)
        if(NOT arg_NAME)
            message(FATAL_ERROR "internal error: z_msys_acquire_msys_declare_package requires argument: NAME")
        endif()
        get_filename_component(filename "${arg_URL}" NAME)
    else()
        if(NOT arg_URL MATCHES [[^https://mirror\.msys2\.org/.*/(([^/]*)-[^-/]+-[^-/]+-[^-/]+\.pkg\.tar\.(xz|zst))$]])
            message(FATAL_ERROR "internal error: regex does not match supplied URL to msys_acquire_msys: ${arg_URL}")
        endif()
        set(filename "msys2-${CMAKE_MATCH_1}")
        if(NOT DEFINED arg_NAME)
            set(arg_NAME "${CMAKE_MATCH_2}")
        endif()
        if(Z_MSYS_${arg_NAME}_DIRECT)
            return()
        endif()
        if(arg_NAME IN_LIST Z_MSYS_PACKAGES_AVAILABLE)
            message(FATAL_ERROR "Redeclaration of package '${arg_NAME}'")
        endif()
    endif()

    list(APPEND Z_MSYS_PACKAGES_AVAILABLE "${arg_NAME}")
    set(Z_MSYS_PACKAGES_AVAILABLE "${Z_MSYS_PACKAGES_AVAILABLE}" CACHE INTERNAL "")
    set(Z_MSYS_${arg_NAME}_URL "${arg_URL}" CACHE INTERNAL "")
    set(Z_MSYS_${arg_NAME}_SHA512 "${arg_SHA512}" CACHE INTERNAL "")
    set(Z_MSYS_${arg_NAME}_FILENAME "${filename}" CACHE INTERNAL "")
    set(Z_MSYS_${arg_NAME}_DEPS "${arg_DEPS}" CACHE INTERNAL "")
    set(Z_MSYS_${arg_NAME}_PATCHES "${arg_PATCHES}" CACHE INTERNAL "")
    set(Z_MSYS_${arg_NAME}_DIRECT "${arg_DIRECT}" CACHE INTERNAL "")
    set(Z_MSYS_${arg_NAME}_PROVIDES "${arg_PROVIDES}" CACHE INTERNAL "")
    foreach(name IN LISTS arg_PROVIDES)
        set(Z_MSYS_${name}_PROVIDED_BY "${arg_NAME}" CACHE INTERNAL "")
    endforeach()
endfunction()
...
function(z_msys_acquire_msys_download_packages)
    ...
    while(NOT backlog STREQUAL "")
        ...
        if(DEFINED Z_MSYS_${name}_PROVIDED_BY AND NOT name IN_LIST Z_MSYS_PACKAGES_AVAILABLE)
            set(name "${Z_MSYS_${name}_PROVIDED_BY}")
            if(name IN_LIST resolved)
                continue()
            endif()
        endif()
        if(NOT name IN_LIST Z_MSYS_PACKAGES_AVAILABLE)
            list(APPEND unknown "${name}")
            continue()
        endif()
        list(APPEND resolved "${name}")
        list(REMOVE_ITEM Z_MSYS_${name}_DEPS ${resolved} ${backlog})
        list(APPEND backlog ${Z_MSYS_${name}_DEPS})

        z_msys_acquire_msys_download_package(archive
            URL "${Z_MSYS_${name}_URL}"
            SHA512 "${Z_MSYS_${name}_SHA512}"
            FILENAME "${Z_MSYS_${name}_FILENAME}"
        )
        set(Z_MSYS_${name}_ARCHIVE "${archive}" CACHE INTERNAL "")
    endwhile()
...
function(z_msys_acquire_msys_collect_hashes out_hash)
    ...
    foreach(name IN LISTS arg_PACKAGES)
        if(NOT DEFINED Z_MSYS_${name}_SHA512)
            message(FATAL_ERROR "SHA512 unknown for '${name}'.")
        endif()
        string(APPEND result "${Z_MSYS_${name}_SHA512}")
        foreach(patch IN LISTS Z_MSYS_${name}_PATCHES)
...
function(msys_acquire_msys out_msys_root)
    ...
    if(DEFINED arg_UNPARSED_ARGUMENTS)
        message(WARNING "msys_acquire_msys was passed extra arguments: ${arg_UNPARSED_ARGUMENTS}")
    endif()

    z_msys_acquire_msys_declare_all_packages()
    set(requested "${arg_PACKAGES}")
    if(arg_Z_ALL_PACKAGES)
        set(requested "${Z_MSYS_PACKAGES_AVAILABLE}")
    ...
    if(DEFINED arg_DIRECT_PACKAGES AND NOT arg_DIRECT_PACKAGES STREQUAL "")
        ...
        if(direct_packages_parity EQUAL 1)
            message(FATAL_ERROR "msys_acquire_msys(... DIRECT_PACKAGES ...) requires exactly pairs of URL/SHA512")
        endif()
        ...
        foreach(index RANGE "${direct_packages_last}")
            ...
            z_msys_acquire_msys_declare_package(
                NAME "${pkg_name}"
                URL "${url}"
                SHA512 "${sha512}"
                DIRECT
    ...
    z_msys_acquire_msys_download_packages(
        PACKAGES ${requested}
        OUT_RESOLVED resolved
        OUT_UNKNOWN unknown
    )
    if(NOT unknown STREQUAL "")
        message(FATAL_ERROR "Unknown packages were required for msys_acquire_msys(${requested}): ${unknown}
This can be resolved by explicitly passing URL/SHA pairs to DIRECT_PACKAGES.")
    endif()
    set(Z_MSYS_PACKAGES_RESOLVED "${resolved}" CACHE INTERNAL "Export for CI")

    z_msys_acquire_msys_collect_hashes(hashes PACKAGES ${resolved})
    ...
    if(NOT EXISTS "${path_to_root}")
        ...
        foreach(name IN LISTS resolved)
            file(ARCHIVE_EXTRACT
                INPUT "${Z_MSYS_${name}_ARCHIVE}"
                DESTINATION "${path_to_root}.tmp"
            )
            if(Z_MSYS_${name}_PATCHES)
                z_apply_patches(
                    SOURCE_PATH "${path_to_root}.tmp"
                    PATCHES ${Z_MSYS_${name}_PATCHES}
                )
            endif()
        endforeach()
