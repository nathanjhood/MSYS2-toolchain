...
function(z_msys_check_hash result file_path sha512)
...
function(z_msys_download_distfile_test_hash file_path kind error_advice sha512 skip_sha512)
    if(_MSYS_INTERNAL_NO_HASH_CHECK)
    ...
    set(hash_match OFF)
    z_msys_check_hash(hash_match "${file_path}" "${sha512}")
...
function(z_msys_download_distfile_show_proxy_and_fail error_code)
    message(FATAL_ERROR
        "    \n"
...
function(z_msys_download_distfile_via_aria)
    ...
    msys_list(SET headers_param)
    foreach(header IN LISTS arg_HEADERS)
        msys_list(APPEND headers_param "--header=${header}")
    endforeach()
    ...
    if (NOT "${error_code}" STREQUAL "0")
        ...
        z_msys_download_distfile_show_proxy_and_fail("${error_code}")
    else()
        z_msys_download_distfile_test_hash(
...
function(msys_download_distfile out_var)
    cmake_parse_arguments(PARSE_ARGV 1 arg
    ...
    if(NOT DEFINED arg_URLS)
        message(FATAL_ERROR "msys_download_distfile requires a URLS argument.")
    endif()
    if(NOT DEFINED arg_FILENAME)
        message(FATAL_ERROR "msys_download_distfile requires a FILENAME argument.")
    endif()
    if(arg_SILENT_EXIT)
        message(WARNING "SILENT_EXIT has been deprecated as an argument to msys_download_distfile -- remove the argument to resolve this warning")
    endif()
    ...
    if(NOT arg_SKIP_SHA512 AND NOT DEFINED arg_SHA512)
        message(FATAL_ERROR "msys_download_distfile requires a SHA512 argument.
If you do not know the SHA512, add it as 'SHA512 0' and re-run this command.")
    elseif(arg_SKIP_SHA512 AND DEFINED arg_SHA512)
        message(FATAL_ERROR "msys_download_distfile must not be passed both SHA512 and SKIP_SHA512.")
    endif()
    ...
    if(EXISTS "${downloaded_file_path}" AND NOT arg_SKIP_SHA512)
        set(hash_match OFF)
        z_msys_check_hash(hash_match "${downloaded_file_path}" "${arg_SHA512}")
    ...
    # Delete "temp0" directory created by the old version of script
    ...
    if(EXISTS "${downloaded_file_path}" AND NOT arg_ALWAYS_REDOWNLOAD)
        ...
        if(NOT msys_download_distfile_QUIET)
            message(STATUS "Using cached ${arg_FILENAME}.")
        endif()

        # Suppress the "Downloading ${arg_URLS} -> ${arg_FILENAME}..." message
        set(msys_download_distfile_QUIET TRUE)
    endif()

    # msys_download_distfile_ALWAYS_REDOWNLOAD only triggers when NOT _MSYS_NO_DOWNLOADS
    ...
    if(NOT arg_DISABLE_ARIA2 AND _MSYS_DOWNLOAD_TOOL STREQUAL "ARIA2" AND NOT EXISTS "${downloaded_file_path}")
        if (arg_SKIP_SHA512)
            set(OPTION_SKIP_SHA512 "SKIP_SHA512")
        endif()
        z_msys_download_distfile_via_aria(
            "${OPTION_SKIP_SHA512}"
    ...
    msys_list(SET urls_param)
    foreach(url IN LISTS arg_URLS)
        msys_list(APPEND urls_param "--url=${url}")
    endforeach()
    if(NOT msys_download_distfile_QUIET)
    ...
    if(arg_SKIP_SHA512)
        msys_list(SET sha512_param "--skip-sha512")
    else()
        msys_list(SET sha512_param "--sha512=${arg_SHA512}")
    endif()

    # This obviously needsa rethink but we might be able to hook it into <DLAGENTS>?
