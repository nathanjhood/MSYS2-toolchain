...
function(z_msys_fixup_pkgconfig_process_data arg_variable arg_config arg_prefix)
    ...
    string(REGEX REPLACE "[ \t]*\\\\\n[ \t]*" " " contents "${contents}")
    # This section fuses XYZ.private and XYZ according to CRT_LINKAGE
    #
    # Pkgconfig searches Requires.private transitively for Cflags in the dynamic case,
    # which prevents us from removing it.
    #
    # Once this transformation is complete, users of msys should never need to pass
    # --static.
...
function(z_msys_fixup_pkgconfig_check_files arg_file arg_config)
    message("Calling ${CMAKE_CURRENT_FUNCTION}")
    ...
    msys_host_path_list(PREPEND ENV{PKG_CONFIG_PATH}
...
function(msys_fixup_pkgconfig)
    ...
    if(NOT DEFINED arg_RELEASE_FILES)
        ...
        foreach(debug_file IN LISTS arg_DEBUG_FILES)
            msys_list(REMOVE_ITEM arg_RELEASE_FILES "${debug_file}")
        endforeach()
    endif()

    foreach(config IN ITEMS RELEASE DEBUG)
        debug_message("${config} Files: ${arg_${config}_FILES}")
        if("${MSYS_BUILD_TYPE}" STREQUAL "release" AND "${config}" STREQUAL "DEBUG")
            continue()
        endif()
        foreach(file IN LISTS "arg_${config}_FILES")
            ...
            z_msys_fixup_pkgconfig_process_data(contents "${config}" "\${pcfiledir}/${relative_pc_path}")
            file(WRITE "${file}" "${contents}")
        endforeach()

        if(NOT arg_SKIP_CHECK) # The check can only run after all files have been corrected!
            msys_find_acquire_program(PKGCONFIG)
            debug_message("Using pkg-config from: ${PKGCONFIG}")
            foreach(file IN LISTS "arg_${config}_FILES")
                z_msys_fixup_pkgconfig_check_files("${file}" "${config}")
            endforeach()
        endif()
    endforeach()
    debug_message("Fixing pkgconfig --- finished")

    set(Z_MSYS_FIXUP_PKGCONFIG_CALLED TRUE CACHE INTERNAL "See below" FORCE)
    # Variable to check if this function has been called!
    # Theoreotically msys could look for *.pc files and automatically call this function
    # or check if this function has been called if *.pc files are detected.
    # The same is true for msys_fixup_cmake_targets
endfunction()
