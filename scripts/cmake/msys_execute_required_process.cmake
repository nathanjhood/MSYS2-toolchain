...
function(msys_execute_required_process)
    ...
    if (MSYS_DOWNLOAD_MODE AND NOT arg_ALLOW_IN_DOWNLOAD_MODE)
        message(FATAL_ERROR
[[
...
Halting portfile execution.
    ...
    if(X_PORT_PROFILE AND NOT arg_ALLOW_IN_DOWNLOAD_MODE)
        msys_list(PREPEND arg_COMMAND "${CMAKE_COMMAND}" "-E" "time")
    endif()
    ...
    msys_list(SET logfiles)
    msys_list(SET logfile_copies)
    set(expect_alias FALSE)
    foreach(item IN LISTS arg_SAVE_LOG_FILES)
        if(expect_alias)
            msys_list(POP_BACK logfile_copies)
            msys_list(APPEND logfile_copies "${CURRENT_BUILDTREES_DIR}/${arg_LOGNAME}-${item}")
            set(expect_alias FALSE)
    ...
    if(NOT error_code EQUAL 0)
        ...
        foreach(log IN LISTS saved_logs ITEMS "${log_out}" "${log_err}")
            ...
            if(NOT log_size EQUAL "0")
                ...
                file(APPEND "${Z_MSYS_ERROR_LOG_COLLECTION_FILE}" "${native_log}\n")
            endif()
        endforeach()

        z_msys_prettify_command_line(pretty_command ${arg_COMMAND})
        message(FATAL_ERROR
    ...
    if(output_and_error_same)
        z_msys_forward_output_variable(arg_OUTPUT_VARIABLE out_err_var)
        # arg_ERROR_VARIABLE = arg_OUTPUT_VARIABLE, so no need to set it again
    else()
        z_msys_forward_output_variable(arg_OUTPUT_VARIABLE out_var)
        z_msys_forward_output_variable(arg_ERROR_VARIABLE err_var)
    endif()
endfunction()
