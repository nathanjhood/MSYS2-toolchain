...
function(msys_copy_tools)
    ...
    foreach(tool_name IN LISTS arg_TOOL_NAMES)
        set(tool_path "${arg_SEARCH_DIR}/${tool_name}${MSYS_TARGET_EXECUTABLE_SUFFIX}")
        set(tool_pdb "${arg_SEARCH_DIR}/${tool_name}.pdb")
        if(EXISTS "${tool_path}")
            file(COPY "${tool_path}" DESTINATION "${arg_DESTINATION}")
        elseif(NOT "${MSYS_TARGET_BUNDLE_SUFFIX}" STREQUAL "" AND NOT "${MSYS_TARGET_BUNDLE_SUFFIX}" STREQUAL "${MSYS_TARGET_EXECUTABLE_SUFFIX}")
            set(bundle_path "${arg_SEARCH_DIR}/${tool_name}${MSYS_TARGET_BUNDLE_SUFFIX}")
    ...
    if(arg_AUTO_CLEAN)
        msys_clean_executables_in_bin(FILE_NAMES ${arg_TOOL_NAMES})
    endif()

    msys_copy_tool_dependencies("${arg_DESTINATION}")
endfunction()
